...
func TestJudgePolicyDeniesFromLocalJudge(t *testing.T) {
	...
	if decision.RiskEvent.DecisionStage != risk.DecisionStageAdvisory || decision.RiskEvent.JudgeModel != "qwen3-0.6b-q4" || decision.RiskEvent.JudgeRiskLevel != "high" {
...
func TestJudgePolicyAllowsFromLocalJudge(t *testing.T) {
	...
	if decision.RiskEvent.DecisionStage != risk.DecisionStageAdvisory || decision.RiskEvent.PolicyVersion != policy.DefaultPolicyVersion {
		t.Fatalf("risk event = %+v", decision.RiskEvent)
	}
}
