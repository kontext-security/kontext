...
const (
	...
	// Retain results that fit the history field so token packing can keep their
	// beginning and end. The total per-session bound remains maxHistoryBytes.
	maxHistoryEntryBytes = maxHistoryBytes
...
func (s *ContextStore) RecordUserRequest(sessionID, request string) {
	...
	ctx.requestTooLarge = len(request) > maxUserRequestBytes
	if ctx.requestTooLarge {
		ctx.request = ""
	} else {
		ctx.request = strings.Clone(request)
	}
}
...
func (s *ContextStore) RecordInteraction(sessionID string, entry HistoryEntry) {
	...
	defer s.mu.Unlock()
	ctx := s.session(sessionID)
	if ExcludedTool(entry.ToolName) {
		ctx.historyOmitted = true
		return
	}
	remaining := maxHistoryEntryBytes
	for _, value := range []any{entry.ToolName, entry.ToolArguments, entry.ToolResponse, entry.Error} {
		if err := boundedJSONSize(value, &remaining, 0); err != nil {
			ctx.historyOmitted = true
			return
		}
	}
	encoded, err := serializeHistory([]HistoryEntry{entry})
	if err != nil || len(encoded) > maxHistoryEntryBytes {
		ctx.historyOmitted = true
		return
	}
	// Store an immutable, bounded serialization, never references to caller
	// maps or large raw tool results. Drop large events before evicting history.
	ctx.history = append(ctx.history, encoded[1:len(encoded)-1])
	for len(ctx.history) > maxHistoryEntries || len(joinHistory(ctx.history)) > maxHistoryBytes {
		ctx.history = append([]string(nil), ctx.history[1:]...)
		ctx.historyOmitted = true
	}
}

func joinHistory(entries []string) string { return "[" + strings.Join(entries, ",") + "]" }
...
func (s *ContextStore) Snapshot(sessionID string) (userRequest, interactionHistory string) {
	snapshot := s.SnapshotWithCoverage(sessionID)
	return snapshot.UserRequest, snapshot.InteractionHistory
}

func (s *ContextStore) SnapshotWithCoverage(sessionID string) ContextSnapshot {
	empty := ContextSnapshot{InteractionHistory: "[]"}
	if s == nil || sessionID == "" {
		return empty
	}
	s.mu.Lock()
	...
	return ContextSnapshot{
		UserRequest: ctx.request, InteractionHistory: joinHistory(ctx.history),
		HistoryOmitted: ctx.historyOmitted, RequestTooLarge: ctx.requestTooLarge,
	}
}
