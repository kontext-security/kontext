...
const (
	...
	ErrorExcludedTool    = "excluded_tool"
	ErrorActionTooLarge  = "action_too_large"
	ErrorRequestTooLarge = "request_too_large"
	ErrorSchemaTooLarge  = "schema_too_large"
	ErrorUnsupportedText = "unsupported_text"
	ErrorInvalidHistory  = "invalid_history"
...
// action stays structured until Go renders the training form.
type Input struct {
	UserRequest          string `json:"user_request"`
	RequestTooLarge      bool   `json:"request_too_large"`
...
func (e *Evaluator) Evaluate(ctx context.Context, input Input) Evaluation {
	...
	result := Evaluation{
		...
		ModelVersion:       ModelVersion,
		Enforced:           false,
		UserRequestPresent: input.RequestTooLarge || strings.TrimSpace(input.UserRequest) != "",
	...
	result.HistoryOmitted = input.HistoryOmitted
	if ExcludedTool(input.ToolName) {
		result.ErrorCode = ErrorExcludedTool
	...
	// Keep the slot until native execution has actually stopped, even when
	// the hook deadline wins. This bounds outstanding work and prevents late
	// results from being attributed to a later request. The native binding does
	// not interrupt Run, so its completion must not extend the hook deadline.
	type outcome struct {
	...
	go func() {
		defer func() { <-e.slots }()
		value, err := e.backend.Infer(callCtx, input)
	...
	if output.err != nil {
		result.ErrorCode = errorCode(output.err)
		return result
	}
	result.HistoryOmitted = result.HistoryOmitted || output.value.HistoryOmitted
...
func validateInputBounds(input Input) error {
	if input.RequestTooLarge || len(input.UserRequest) > maxPretokenizedFieldBytes || len(input.InteractionHistory) > maxPretokenizedFieldBytes || len(input.ToolName) > 256 {
		return backendError(ErrorInputTooLarge, nil)
	}
	for _, value := range []any{input.ToolArguments, input.AvailableToolSchemas} {
		remaining := maxPretokenizedFieldBytes
