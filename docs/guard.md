...
On a self-serve Mac, `kontext setup --with-local-llm` is the supported way to opt in. It checks `llama-server` is on PATH **before** writing anything, so wanting the model without having installed `llama.cpp` costs nothing to recover from; it writes `KONTEXT_JUDGE_MANAGED=1` into the LaunchAgent's environment, since launchd already owns the daemon's environment and the daemon reads exactly that variable; and it pre-fetches the weights afterwards, so the ~680 MB download and multi-minute first load happen behind a progress bar rather than inside a developer's first tool call. A failed pre-fetch warns and continues — the weights are an optimization of something that already degrades cleanly. Without the flag, the install is byte-identical to one made before the option existed.
